...
float output[3];
float target[5] = {400,200,60,60};
float target_single[5] = {400,40,60,60};
...
void OnScreenObjectCallback(const std_msgs::Int32MultiArray::ConstPtr &msg)
{
	...
	ROS_INFO("msg.data[2] = %d\n",msg->data[2]);
		...
		//single person
		docc.setControlTargets(target_single);
		controlIn[0] = (msg->data[0]  + msg->data[0])/2.0f;
		controlIn[1] = msg->data[2];
		...
		//adjust PIDs
		docc.pid_forward_v.setPID(10,0.0000,0.001);
		
		if(abs(docc.pid_forward_v.error)<5){
			docc.pid_forward_v.setPID(0.2,0.0000,0.003);
		}
		else{
			docc.pid_forward_v.setPID(2,0.0000,0.003);
		}
		


		if(abs(docc.pid_objDistS_v.error)<8){
			docc.pid_objDistS_v.setPID(0.001,0.0000,0.001);
		}
		else{
			docc.pid_objDistS_v.setPID(0.01,0.0000,0.001);
		}
	...
	ROS_INFO("msg.data[5] = %d\n",msg->data[5]);
		...
		//adjust PIDs
		if(abs(docc.pid_forward_v.error)<300){
